...
# contains data(username), header, logged-in(time), last-active(time) and private-connection(p2p)
online_clients = {}

# List of blocked clients socket as key
# contains data(username) and account-blocked(time)
blocked_clients = {}

# List of disconnected clients: socket as key
# contains the same as online_clients
offline_clients = {}

# List of offline messages: socket as key
# contains an array/ list of dict(offline messages)
offline_messages = {}
...
while (1):
    ...
    for notified_socket in list(online_clients):
        if 'last-active' in online_clients[notified_socket]:
            if minus_timeout == online_clients[notified_socket]['last-active'] or minus_timeout > online_clients[notified_socket]['last-active']:
                print('\nConnection timeout for: {} since {}'.format(online_clients[notified_socket]['data'].decode().split(',')[0], (online_clients[notified_socket]['last-active'] + datetime.timedelta(seconds=timeout)).strftime("%d/%m/%Y, %H:%M:%S.%f")[:-3]))
                server = 'WICKWICK\'S SERVER'.encode()
                server_header = f"{len(server):<{20}}".encode()
                message = 'Connection timeout at {} due to inactivity. Bye {}!'.format((online_clients[notified_socket]['last-active'] + datetime.timedelta(seconds=timeout)).strftime("%d/%m/%Y, %H:%M:%S.%f")[:-3], online_clients[notified_socket]['data'].decode()).encode()
    ...
    for notified_socket in read_sockets:
        ...
        if notified_socket is server_socket:
            ...
            print(f'client_socket: {client_socket.getsockname()}')
        ...
        # server will reuse message header sent by sender for certain commands like message, broadcasting, etc
            ...
            if command == 'broadcast':
                if length_encoded_msg(encoded_msg=message['data']) >= 2:
                    ...
                    if blocked:
                        message = 'Some user will not get the broadcast!'.encode()
                        message_header = f"{len(message):<{20}}".encode()
            ...
            # --- whoelse start ---
                ...
                if length_encoded_msg(encoded_msg=message['data']) == 1:
                    ...
                    message = 'whoelse {}'.format(len(list(online_clients))).encode()
                    message_header = f"{len(message):<{20}}".encode()
                    notified_socket.send(user['header'] + user['data'] + message_header + message)
                            ...
                            for client_socket in offline_clients:
                                if client_socket != notified_socket and offline_clients[client_socket]['data'].decode() == recipient:
                                    ...
                                    if not user_blocked(my_socket=notified_socket, on9clients=online_clients, off9clients=offline_clients):
                                        msg = msg.strip().encode()
                                        # store the offline messages in a list og dicts
            ...
            # --- P2P commands start---
                ...
                if length_encoded_msg(encoded_msg=message['data']) == 2:
                    ...
                    if username == user['data'].decode():
                        #FIXME uncomment debugging print before submitting
                        # print('{} can\'t UNBLOCK SELF!'.format(username))
                        message = 'Error, can\'t startprivate self: {}!'.format(username).encode()
                        message_header = f"{len(message):<{20}}".encode()
                        notified_socket.send(user['header'] + user['data'] + message_header + message)
                        break
                    # check if user exist    
                    if user_exists(username=username, on9clients=online_clients, off9clients=offline_clients):
                        # check if user is online and not offline
                        if user_exists_On9clients(username=username, on9clients=online_clients) and not user_exists_Off9clients(username=username, off9clients=offline_clients):
                            for client_socket in online_clients:
                                if client_socket != notified_socket:
                                    if online_clients[client_socket]['data'].decode() == username:
                                        if 'blocked-user' in user:
                                            if user_blocked_list(my_socket=notified_socket, to_check_socket=client_socket, on9clients=online_clients): 
                                                #FIXME uncomment debugging print before submitting
                                                # print('FAIL: can\'t startprivate with {}!'.format(username))
                                                message = f'Error, can\'t startprivate with {username}, either {username} is blocked or {username} blocked you!'.encode()
                                                message_header = f"{len(message):<{20}}".encode()
                                                notified_socket.send(user['header'] + user['data'] + message_header + message)
                                                break
                                        if 'blocked-user' in online_clients[client_socket]:
                                            if user_blocked_list(my_socket=client_socket, to_check_socket=notified_socket, on9clients=online_clients):
                                                ...
                                                notified_socket.send(user['header'] + user['data'] + message_header + message)
                                                break
                                        # to init p2p send to both party
                                        message = 'Waiting for {} to accept your connection...'.format(online_clients[client_socket]['data'].decode()).encode()
                                        message_header = f"{len(message):<{20}}".encode()
                                        notified_socket.send(user['header'] + user['data'] + message_header + message)
                                        message = 'startprivate {} {} {} {}'.format(online_clients[client_socket]['private-connection'][0], online_clients[client_socket]['private-connection'][1], user['data'].decode(), online_clients[client_socket]['data'].decode()).encode()
                                        message_header = f"{len(message):<{20}}".encode()
                                        notified_socket.send(user['header'] + user['data'] + message_header + message)
                                        # the other party will need to accept by pressing enter
                                        message = 'startprivate {} {} {} {}'.format(user['private-connection'][0], user['private-connection'][1], online_clients[client_socket]['data'].decode(),user['data'].decode()).encode()
                                        message_header = f"{len(message):<{20}}".encode()
                                        client_socket.send(online_clients[client_socket]['header'] + online_clients[client_socket]['data'] + message_header + message)
                                        break
                        else:
                            #FIXME uncomment debugging print before submitting
                            # print('FAIL: {} not online!'.format(username))
                            message = 'Error, {} not online!'.format(username).encode()
                            message_header = f"{len(message):<{20}}".encode()
                            notified_socket.send(user['header'] + user['data'] + message_header + message)
                            break
                    # if user in credentials.txt but never log in
