...
echo "set term postscript color; set output \"$outname\_delay.ps\"; set xlabel \"Packet Number\"; set ylabel \"Delay (ms)\"; set timestamp; plot $plot" | gnuplot - > /dev/null 2> /dev/null
echo "ps2pdf "$outname"_delay.ps"
ps2pdf "$outname"_delay.ps

echo "set term postscript color; set output \"$outname\_scatter.ps\"; set xlabel \"Packet Size (bytes)\"; set ylabel \"Delay (ms)\";  set timestamp; plot $plot2, \"$outavg\" t \"avg\" w l -1 " | gnuplot - > /dev/null 2> /dev/null 
